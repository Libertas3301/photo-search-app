...
export default class IndexPage extends react.Component {
  ...
  onSearchSubmit = async (value) => {
    ...
    const response = await axios.get("https://api.unsplash.com/search/photos", {
      params: { query: value },
      headers: {
        Authorization: "Client-ID JoBf5D3FDF5Z90j9hNksfxZBPg3g5Y2h79hzRtRNQF0",
      },
    }).then(
      setTimeout(function () {
                ...
                this.state.photosData.map((item, index) => {
                  return (
                    <div key={index} onClick={this.openInNewPage}>
                      <img src={item.urls.small} alt={item.alt_description} onMouseOver={this.handleLinkChange} />
                      <p className={styles.likes__counter}>{item.likes} likes</p>
                    </div>
                  );
                })}
            </div>
              ...
              <div className={styles.query__data}>
                ...
                <ul className={styles.query__element}>
                  {this.state.savedQueries.map((item, index) => (
                    <li onClick={this.getPhotosByQuery} key={index} className={styles.query__entity}>{item}</li>
                  ))}
                </ul>
              </div>}
          </div>}
